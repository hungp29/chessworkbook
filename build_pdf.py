...
class ChessWorkbookDoc(BaseDocTemplate):
    ...
    def draw_header_footer(self, canvas, doc):
        ...
        chapter_name = self.page_chapters.get(page)
...
def build_story(insert_blank_after_toc=False):
    ...
    for chapter_index, chapter in enumerate(chapters):
        ...
        story.append(
            Paragraph(
                f"{chapter_index + 1}. {chapter_name}",
                chapter_style,
            )
        )
...
original_chapter_start_pages = temp_doc.chapter_start_pages.copy()
original_page_chapters = temp_doc.page_chapters.copy()

toc_last_page = temp_doc.toc_last_page

print("chapter_start_pages =", sorted(chapter_start_pages))

print("page_chapters =", sorted(page_chapters.items()))

print("toc_last_page =", toc_last_page)

page_chapters = original_page_chapters.copy()

last_chapter = None
max_page = max(page_chapters)
for page in range(1, max_page + 1):
    if page in page_chapters:
        last_chapter = page_chapters[page]
    elif last_chapter:
        page_chapters[page] = last_chapter
...
#
# Dịch page mapping từ PASS 1 sang PASS 2
#

page_offset = content_start_page - 1

chapter_start_pages = {

    p + page_offset

    for p in original_chapter_start_pages

}

page_chapters = {p + page_offset: chapter for p, chapter in page_chapters.items()}

print("FINAL chapter_start_pages =", sorted(chapter_start_pages))

print("FINAL page_chapters =", sorted(page_chapters.items())[:50])


print("content_start_page =", content_start_page)
...
final_doc.chapter_start_pages = chapter_start_pages
final_doc.page_chapters = page_chapters
